...
func NewBDDContext(t *testing.T) *BDDContext {
	...
	server := api.NewServer(database, port, 10*time.Second)
...
func (c *BDDContext) Shutdown() {
	...
	c.apiServer.Shutdown(shutdownCtx)
	shutdownCancel()
	...
	if c.worker != nil {
		workerShutdownCtx, workerShutdownCancel := context.WithTimeout(context.Background(), 1*time.Second)
		c.worker.Shutdown(workerShutdownCtx)
...
func (c *BDDContext) startScheduler() {
	c.t.Helper()
	c.sched = scheduler.NewScheduler(c.database, 100*time.Millisecond)
	c.wg.Add(1)
	go func() {
		defer c.wg.Done()
		c.sched.Start(c.ctx)
...
func (c *BDDContext) startWorker() {
	c.t.Helper()
	c.worker = worker.NewWorker(c.workerID, c.apiClient)
	c.wg.Add(1)
	go func() {
		defer c.wg.Done()
...
func (c *BDDContext) WhenIWaitForJobToComplete(jobID string, timeout time.Duration) *BDDContext {
	...
	for time.Since(start) < timeout {
		job, err := c.database.GetJob(jobID)
		if err != nil {
...
// MustGetJob retrieves the job from the database, failing the test if it cannot be found.
func (c *BDDContext) MustGetJob(jobID string) *models.Job {
	c.t.Helper()
	job, err := c.database.GetJob(jobID)
	if err != nil {
		c.t.Fatalf("Failed to retrieve job %s: %v", jobID, err)
	}
	if job == nil {
		c.t.Fatalf("Job %s not found in database", jobID)
	}
	return job
}
...
func TestIntegration_WorkerGracefulShutdown(t *testing.T) {
	...
	// WHEN: I initiate a graceful shutdown on the worker immediately.
	// We use a separate context for the shutdown operation with a timeout.
	shutdownCtx, cancelShutdown := context.WithTimeout(context.Background(), 7*time.Second)
	defer cancelShutdown()

	workerDone := make(chan struct{})
	go func() {
		ctx.worker.Shutdown(shutdownCtx)
		close(workerDone)
	}()
