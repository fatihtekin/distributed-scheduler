...
func NewWorker(id string, client *api.Client) *Worker {
	defaultInterval := 2 * time.Second
	w := &Worker{
		id:                  id,
		defaultPollInterval: defaultInterval,
		jobConcurrencyGate:  make(chan struct{}, MaxConcurrentJobs),
		client:              client,
		runningJobs:         &sync.WaitGroup{},
	}

	w.executors = map[models.JobType]executors.Executor{
...
func (w *Worker) Start(ctx context.Context) {
	log.Printf("Worker %s started with max concurrency %d, polling API",
		w.id, MaxConcurrentJobs)
...
func (w *Worker) pollForJobs(ctx context.Context) {
	for {
		...
		case w.jobConcurrencyGate <- struct{}{}:
			job, err := w.client.GetNextJob(w.id)
			if err != nil {
...
func (w *Worker) run(ctx context.Context, job *models.Job) {
	...
	log.Printf("Worker %s: Processing job %s (type: %s, priority: %d)",
