...
func NewServer(database db.DBClient, port int, jobTimeOutDuration time.Duration) *Server {
	s := &Server{
		db:                 database,
...
func (s *Server) getNextJob(w http.ResponseWriter, r *http.Request) {
	...
	job, err := s.db.GetNextJob(workerID, s.jobTimeOutDuration)
	if err != nil {
	...
	if job == nil {
		respondJSON(w, http.StatusNotFound, map[string]string{"message": "No jobs available"})
		return
	}
...
func (s *Server) completeJob(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.db.CompleteJob(id, req.Status); err != nil {
